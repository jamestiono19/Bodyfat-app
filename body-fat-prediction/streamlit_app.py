...
if predict_clicked:
    ...
    for i in range(100):
        if i >= next_threshold:
            ...
            try:
                ...
                next_threshold = 101
        latency_text.markdown(
            f"""<div style="text-align:center; font-size:12px; color:#475569;
                font-family:monospace; letter-spacing:.6px; padding-bottom:4px;">
                ⏱ model latency &nbsp;<span style="color:#60a5fa; font-weight:600;">{_infer_ms} ms</span>
            </div>""",
            unsafe_allow_html=True,
        )
        time.sleep(0.005)
    ...
    .latency-badge {{
        ...
        backdrop-filter: blur(12px);
        box-shadow: 0 4px 18px rgba(0,0,0,0.45), 0 0 12px rgba(96,165,250,0.15);
        font-family: 'Courier New', monospace;
        font-size: 12.5px;
        color: #94a3b8;
        letter-spacing: 0.5px;
        animation: badgePop 0.4s cubic-bezier(.175,.885,.32,1.275) both;
    }}
    .latency-badge .badge-dot {{
    ...
    <div class="latency-badge">
        ...
        ⏱ model latency &nbsp;<span class="badge-val">{_infer_ms} ms</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if not os.path.exists(coeff_path):
            ...
            coeff_df["Percentage"] = (coeff_df["AbsoluteCoefficient"] / coeff_df["AbsoluteCoefficient"].sum()) * 100
